...
if [ "$USE_SUPERVISOR" = true ]; then
    ...
    # Window 7: CEO Scheduler (pure bash, zero tokens)
    tmux new-window -t "$SESSION_NAME" -n "ceo-sched" \
        "cd $REPO_ROOT && bash scripts/ceo-scheduler.sh; read -p 'CEO Scheduler stopped. Press Enter...'"

    # Window 8: Logs overview
    tmux new-window -t "$SESSION_NAME" -n "logs" \
